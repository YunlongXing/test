...
func trace_comm(objs bpfObjects) {
	...
	for {
		...
		if event.EventId == EVENT_COMM {
			...
			_, has := commMap[data]
			if !has {
				log.Println("[OPEN_FILE]", string(event.Log[:]), event.Val)
				openSnoop.WriteString(string(event.Log[:]) + "\n")
			}
			commMap[data] = int(event.Val)
